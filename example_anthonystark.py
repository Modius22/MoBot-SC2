#anthonystarke/Python-StarCraft2-AI

import sc2
from sc2 import run_game, maps, Race, Difficulty, position
from sc2.player import Bot, Computer
from sc2.constants import NEXUS, PROBE, PYLON, GATEWAY, ZEALOT, ASSIMILATOR, CYBERNETICSCORE, STALKER, PHOTONCANNON, \
    FORGE, STARGATE, DARKSHRINE, VOIDRAY, TWILIGHTCOUNCIL, DARKTEMPLAR, \
    OBSERVER, ROBOTICSFACILITY, RESEARCH_PROTOSSGROUNDWEAPONS, RESEARCH_PROTOSSGROUNDARMOR, RESEARCH_PROTOSSSHIELDS, \
...
class SentdeBot(sc2.BotAI):
    ...
    async def upgrade_system(self):
        if self.units(FORGE).ready.noqueue and self.WEAPONS_UPGRADE < 3:
            ...
            elif self.can_afford(RESEARCH_PROTOSSGROUNDWEAPONS) and self.WEAPONS_UPGRADE == 1 and self.units(
                    TWILIGHTCOUNCIL).ready:
                for f in self.units(FORGE).ready.noqueue:
                    await self.do(f(RESEARCH_PROTOSSGROUNDWEAPONS))
                    self.WEAPONS_UPGRADE == 2
            elif self.can_afford(RESEARCH_PROTOSSGROUNDWEAPONS) and self.WEAPONS_UPGRADE == 2 and self.units(
                    TWILIGHTCOUNCIL).ready:
                for f in self.units(FORGE).ready.noqueue:
                    await self.do(f(RESEARCH_PROTOSSGROUNDWEAPONS))
        ...
        if self.units(FORGE).ready.noqueue and self.ARMOR_UPGRADE < 3:
            ...
            elif self.can_afford(RESEARCH_PROTOSSGROUNDARMOR) and self.ARMOR_UPGRADE == 1 and self.units(
                    TWILIGHTCOUNCIL).ready:
                for f in self.units(FORGE).ready.noqueue:
                    await self.do(f(RESEARCH_PROTOSSGROUNDARMOR))
                    self.ARMOR_UPGRADE == 2
            elif self.can_afford(RESEARCH_PROTOSSGROUNDARMOR) and self.ARMOR_UPGRADE == 2 and self.units(
                    TWILIGHTCOUNCIL).ready:
                for f in self.units(FORGE).ready.noqueue:
                    await self.do(f(RESEARCH_PROTOSSGROUNDARMOR))
        ...
        if self.units(FORGE).ready.noqueue and self.SHIELD_UPGRADE < 3:
            ...
            elif self.can_afford(RESEARCH_PROTOSSSHIELDS) and self.SHIELD_UPGRADE == 1 and self.units(
                    TWILIGHTCOUNCIL).ready:
                for f in self.units(FORGE).ready.noqueue:
                    await self.do(f(RESEARCH_PROTOSSSHIELDS))
                    self.SHIELD_UPGRADE == 2
            elif self.can_afford(RESEARCH_PROTOSSSHIELDS) and self.SHIELD_UPGRADE == 2 and self.units(
                    TWILIGHTCOUNCIL).ready:
                for f in self.units(FORGE).ready.noqueue:
                    await self.do(f(RESEARCH_PROTOSSSHIELDS))
        ...
        if self.units(CYBERNETICSCORE).ready.noqueue and self.AIR_WEAPONS_UPGRADE < 3:
            ...
            elif self.can_afford(RESEARCH_PROTOSSAIRARMOR) and self.AIR_WEAPONS_UPGRADE == 1 and self.units(
                    TWILIGHTCOUNCIL).ready:
                for cc in self.units(CYBERNETICSCORE).ready.noqueue:
                    await self.do(cc(RESEARCH_PROTOSSAIRARMOR))
                    self.AIR_WEAPONS_UPGRADE == 2
            elif self.can_afford(RESEARCH_PROTOSSAIRARMOR) and self.AIR_WEAPONS_UPGRADE == 2 and self.units(
                    TWILIGHTCOUNCIL).ready:
                for cc in self.units(CYBERNETICSCORE).ready.noqueue:
                    await self.do(cc(RESEARCH_PROTOSSAIRARMOR))
        ...
        if self.units(CYBERNETICSCORE).ready.noqueue and self.AIR_ARMOR_UPGRADE < 3:
            ...
            elif self.can_afford(RESEARCH_PROTOSSAIRWEAPONS) and self.AIR_ARMOR_UPGRADE == 1 and self.units(
                    TWILIGHTCOUNCIL).ready:
                for cc in self.units(CYBERNETICSCORE).ready.noqueue:
                    await self.do(cc(RESEARCH_PROTOSSAIRWEAPONS))
                    self.AIR_ARMOR_UPGRADE == 2
            elif self.can_afford(RESEARCH_PROTOSSAIRWEAPONS) and self.AIR_ARMOR_UPGRADE == 2 and self.units(
                    TWILIGHTCOUNCIL).ready:
                for cc in self.units(CYBERNETICSCORE).ready.noqueue:
                    await self.do(cc(RESEARCH_PROTOSSAIRWEAPONS))
    ...
    async def scout(self):
        if self.units(PYLON).ready.exists:
            ...
            location = pylon.center.towards(self.enemy_start_locations[randint(0, len(self.enemy_start_locations) - 1)],
                                            random.randrange(5, 10))
            if len(self.units(NEXUS)) > 1:
                if len(self.units(OBSERVER)) < 3 and len(self.units(PYLON)) > 0:
                    if len(self.units(GATEWAY)) < 1 and self.units(PYLON).ready.exists and self.can_afford(
                            GATEWAY) and not self.already_pending(GATEWAY):
                        self.BUILDING_COUNTER += 1
                        await self.build(GATEWAY, near=location)
                    elif self.units(GATEWAY).ready.exists and not self.units(CYBERNETICSCORE) and self.can_afford(
                            CYBERNETICSCORE) and not self.already_pending(CYBERNETICSCORE):
                        self.BUILDING_COUNTER += 1
                        await self.build(CYBERNETICSCORE, near=location)
                    elif self.units(CYBERNETICSCORE).ready.exists and not self.units(
                            ROBOTICSFACILITY) and self.can_afford(ROBOTICSFACILITY) and not self.already_pending(
                            ROBOTICSFACILITY):
                        self.BUILDING_COUNTER += 1
        ...
        if len(self.units(NEXUS).ready) > 1 and len(self.units(PYLON)) < len(self.units(NEXUS).ready):
            if self.supply_left < 10 and self.supply_cap < 200:
                ...
                if self.can_afford(PYLON) and self.PENDING_PYLONS < 2:
                    location = nexus_loc.center.towards(
                        self.enemy_start_locations[randint(0, len(self.enemy_start_locations) - 1)],
            ...
            if self.supply_left < 10 and self.supply_cap < 200:
                ...
                if self.can_afford(PYLON) and self.PENDING_PYLONS < 2:
                    location = pylon_loc.center.towards(
                        self.enemy_start_locations[randint(0, len(self.enemy_start_locations) - 1)],
                        random.randrange(8, 13))
                    await self.build(PYLON, near=location)
                    self.PENDING_PYLONS += 1
    ...
    async def offensive_force_buildings(self):
        ...
        if self.units(PYLON).ready.exists:  # and self.BUILDING_COUNTER < (len(self.units(NEXUS).ready) * 8):
            ...
            if len(self.units(NEXUS)) > 2 and not self.already_pending(CYBERNETICSCORE) and len(
                    self.units(CYBERNETICSCORE)) < 1:
            ...
            if len(self.units(NEXUS).ready) > 1 or self.minerals > 300:
                if self.can_afford(GATEWAY) and not self.already_pending(GATEWAY) and (
                        len(self.units(GATEWAY).ready) < (len(self.units(NEXUS).ready) * 3)):
                    await self.build(GATEWAY, near=pylon)
                    self.BUILDING_COUNTER += 1

            if len(self.units(NEXUS).ready) > 2:
                if len(self.units(GATEWAY).ready) > 1 and len(self.units(STARGATE)) < (
                        len(self.units(NEXUS).ready) * 3) and self.can_afford(STARGATE) and not self.already_pending(
                        STARGATE):
                    await self.build(STARGATE, near=pylon)
                    self.BUILDING_COUNTER += 1

                if len(self.units(STARGATE).ready) > 1 and len(self.units(ROBOTICSFACILITY)) < (
                        len(self.units(NEXUS).ready) * 2) and self.can_afford(
                        ROBOTICSFACILITY) and not self.already_pending(ROBOTICSFACILITY):
                    await self.build(ROBOTICSFACILITY, near=pylon)
                    self.BUILDING_COUNTER += 1

                if len(self.units(STARGATE).ready) > 1 and not self.units(TWILIGHTCOUNCIL) and not self.already_pending(
                        TWILIGHTCOUNCIL):
                    await self.build(TWILIGHTCOUNCIL, near=pylon)
                    self.BUILDING_COUNTER += 1

                if self.units(TWILIGHTCOUNCIL).ready.exists and not self.units(DARKSHRINE) and not self.already_pending(
                        DARKSHRINE):
        ...
        for gw in self.units(GATEWAY).ready.noqueue:
            if len(self.units(NEXUS)) < 2:
                ...
                if not self.units(CYBERNETICSCORE):
                    ...
                    if self.can_afford(STALKER) and self.supply_left > 0 and len(self.units(STALKER)) < len(
                            self.units(NEXUS)) * 5:
    ...
    async def Attack_units_counter(self):
        self.ATTACK_UNITS = len(self.units(ZEALOT)) + len(self.units(STALKER)) + len(self.units(VOIDRAY)) + len(
