...
class MoBot(sc2.BotAI):
    ...
    async def grow_offensive_buildings(self):
        ...
        if not (len(self.units(SPAWNINGPOOL)) >= (
                self.iteration / (self.ITERATIONS_PER_MINUTE / 2)) or self.already_pending(SPAWNINGPOOL)):
